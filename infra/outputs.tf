...
output "loki_bucket_arn" {
  description = "ARN of the S3 bucket created for Loki object storage."
  value       = var.enable_loki_bucket ? aws_s3_bucket.loki_logs[0].arn : null
}

output "loki_irsa_role_arn" {
  description = "ARN of the IAM role that the Loki service account can assume via IRSA."
  value       = var.enable_loki_bucket ? aws_iam_role.loki_s3[0].arn : null
}
